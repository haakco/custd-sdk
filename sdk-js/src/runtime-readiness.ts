...
function validateText(name: string, value: unknown): asserts value is string {
  if (!nonEmpty(value)) throw new Error(`Custd readiness requires ${name}`);
}
...
function validateOptions(options: RuntimeReadinessOptions): void {
...
async function issueToken(fetchImpl: typeof fetch, credential: RuntimeReadinessOAuthConfig): Promise<string> {
  ...
  if (!token || typeof token !== "object" || !nonEmpty((token as { access_token?: unknown }).access_token)) {
...
function accountTenant(account: unknown): string | undefined {
  if (!account || typeof account !== "object") return undefined;
  const record = account as { companySlug?: unknown; tenant?: { slug?: unknown } };
  if (nonEmpty(record.companySlug)) return record.companySlug;
...
async function verifyTenantBinding(
  fetchImpl: typeof fetch,
  baseUrl: string,
  credential: RuntimeReadinessOAuthConfig,
  token: string,
  tenantSlug: string,
): Promise<void> {
  const response = await requireOk(
    fetchImpl,
    `${baseUrl}/api/v1/account/me`,
    { headers: { Authorization: `Bearer ${token}` } },
    `credential "${credential.name}" tenant binding`,
  );
...
function schemaVersions(body: unknown): unknown[] {
  if (Array.isArray(body)) return body;
  if (body && typeof body === "object") {
    const record = body as { versions?: unknown; schemas?: unknown };
    if (Array.isArray(record.versions)) return record.versions;
    if (Array.isArray(record.schemas)) return record.schemas;
  }
  return [];
}

async function verifyActiveSchema(
  fetchImpl: typeof fetch,
  schemaUrl: string,
  token: string,
  eventTypeSlug: string,
  ...
  const selected = versions.find(
    (version) => version && typeof version === "object" && (version as { version?: unknown }).version === schemaVersion,
  ) as { isActive?: unknown; enabled?: unknown } | undefined;
...
export async function checkRuntimeReadiness(options: RuntimeReadinessOptions): Promise<RuntimeReadinessResult> {
  validateOptions(options);
  const fetchImpl = options.fetch ?? globalThis.fetch;
  if (typeof fetchImpl !== "function") throw new Error("Custd readiness requires fetch");
  ...
  const issued: Array<{ credential: RuntimeReadinessOAuthConfig; token: string }> = [];
  for (const credential of options.oauth) {
