...
describe("client setup reporting-pack manifests", () => {
  it("sends reporting packs through the one manifest apply endpoint", async () => {
    ...
    await client.apply("acme/tenant", manifest, { idempotencyKey: "setup-acme-1" });

    expect(request).toHaveBeenCalledWith("PUT", "/tenant-manifest/acme%2Ftenant", manifest, {
      idempotencyKey: "setup-acme-1",
    });
  });
  ...
  it("rejects duplicate pack keys, invalid revisions, and credentials before transport", async () => {
    ...
    await expect(client.apply("acme", duplicate, { idempotencyKey: "setup-acme-1" })).rejects.toThrow(
      "duplicate reporting pack",
    );
  ...
  it("rejects a blank apply idempotency key before transport", async () => {
    const request = vi.fn();

    await expect(
      new ClientSetupClient(request).apply(
        "acme",
        { reportingPacks: [{ definition: reportingPack }] },
        {
          idempotencyKey: " ",
        },
      ),
  ...
  it("applies once and polls readiness until the runtime activates the pack", async () => {
    ...
    const result = await client.applyAndWait("acme", manifest, {
      idempotencyKey: "setup-acme-1",
      timeoutMs: 100,
      intervalMs: 1,
    });
    ...
    expect(result.readiness.operation?.idempotencyKey).toBe("setup-acme-1");
    expect(result.readiness.operation?.completedAt).toBe("2026-08-26T00:00:01Z");
  ...
  it("times out with the last actionable readiness code", async () => {
    ...
    await expect(
      client.applyAndWait("acme", manifest, { idempotencyKey: "setup-acme-1", timeoutMs: 5, intervalMs: 1 }),
    ).rejects.toThrow(
