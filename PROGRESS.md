...
**Status:** Milestones 1–5 complete and merged to `main`. Version `1.8.1`
...
### M5 — Release
- VERSION bumped to 1.8.1 across root `VERSION`, `sdk-go/VERSION`,
  `sdk-js/package.json`, `sdk-php/composer.json`,
  `sdk-python/pyproject.toml`, `wordpress-plugin/custd.php`.
- Release approval was provided on 2026-08-01. The final tag is `v1.8.1`
  because `v1.8.0` predated the merged browser module installer correction.
...
- Tag `v1.8.1` → wait for `ci.yml` and `release-mirrors.yml` → verify the
  published artifacts → pin Custd consumers to `v1.8.1`.
